          ...
          return (
            <InlineDropdown
              trigger={(open, toggle) => (
                ...
                >
                  <button onClick={toggle}
                    className={`flex items-center gap-1 px-2 py-1 rounded-lg w-full justify-end transition-colors ${(pItem?.value === 'Very Urgent' || pItem?.value === 'Urgent') ? (open ? 'bg-red-200' : 'bg-red-100 hover:bg-red-200') : (open ? 'bg-gray-100' : 'hover:bg-gray-50')}`}>
