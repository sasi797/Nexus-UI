...
function BookingRow({ booking, agents, myUserEmail, bookingConfig, onMarkRead, highlighted, readOnly = false }: {
  ...
  function handleAssignAgent(agent_id: string | null, done: () => void) {
    ...
    updateBooking({ id: booking.id, body: { subject: booking.subject, sender_email: booking.sender_email, agent_id } }).then(() => {
  ...
  const isIgnored = booking.status.toLowerCase() === 'ignored';
        ...
        <Link href={`/dashboard/my-bookings/${booking.id}`} className="block" onClick={() => { sessionStorage.setItem('bts:scroll-y', String(document.getElementById('main-scroll')?.scrollTop ?? 0)); sessionStorage.setItem('bts:last-booking', booking.id); }}>
          ...
          <p className="text-[12px] font-semibold text-gray-900 group-hover:text-indigo-700 transition-colors leading-snug truncate flex items-center gap-1.5">
            ...
            {booking.parent_booking_id && (
              <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-violet-50 border border-violet-200 text-[9px] font-bold text-violet-500 shrink-0">↑ child</span>
            )}
            {booking.has_children && !booking.parent_booking_id && (
              <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-violet-50 border border-violet-200 text-[9px] font-bold text-violet-500 shrink-0">↓ parent</span>
            )}
          </p>
        </Link>
          ...
          return readOnly ? (
            ...
            <InlineDropdown direction="auto"
              trigger={(open, toggle) => (
                ...
                >
                  <button onClick={toggle}
                    className={`flex items-center gap-1 px-2 py-1 rounded-lg w-full justify-end transition-colors ${(pItem?.label === 'Very Urgent' || pItem?.label === 'Urgent') ? (open ? 'bg-red-200' : 'bg-red-100 hover:bg-red-200') : (open ? 'bg-gray-100' : 'hover:bg-gray-50')}`}>
                    <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-gray-900">
                      <svg className={`w-3 h-3 shrink-0 ${pc.text}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={P_PATH[pItem?.label ?? ''] ?? P_PATH['Not Urgent']} />
                      </svg>
...
export default function AllBookingsPage() {
  ...
  const TAB_COUNTS: Record<Tab, number | undefined> = debouncedSearch ? {
    ...
    All:           cntAll?.total,
    Pending:       cntPend?.total,
    'In Progress': cntProg?.total,
    Completed:     cntDone?.total,
    Ignored:       cntIgn?.total,
  };
  ...
  return (
    <motion.div variants={pageTransition} initial="hidden" animate="visible" className="flex flex-col gap-3 h-full min-h-0">
      ...
      <motion.div variants={staggerItem} className="flex items-center gap-2">
        ...
        {/* Left: status tabs — underline style */}
